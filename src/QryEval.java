...
public class QryEval {
	...
	public static DocLengthStore  doclen;
	...
	public static void main(String[] args) throws Exception {
		...
		doclen = new DocLengthStore(READER);
		if (params.get("retrievalAlgorithm").equals("UnrankedBoolean")) {
		...
		} else if (params.get("retrievalAlgorithm").equals("RankedBoolean"))
		 {
			...
			mu = Integer.parseInt(params.get("Indri:mu")); 	
		}
	...
	static Qryop parseQuery(String qString) throws IOException {
		...
		System.out.println("good +"+qString);
		...
		// Tokenize the query.
		System.out.println("machongshen+ "+qString);
...
//			if (!token.equals(")")&&!token.equals("(")&&token.length() !=0 && model instanceof RetrievalModelBM25)
//			{
//				token = "#sum("+token+")";
//			}
			if (token.matches("[ ,(\t\n\r]")) {
				...
				if (tokenizeQuery(token).length != 0) {
					...
					if (token.indexOf(".url") != -1) {
						...
						//System.out.println("machongshen"+tokenizeQuery(token)[0]);
						currentOp.add(new QryopIlTerm(tokenizeQuery(token)[0]));
					}
				} else {
	...
	static String printResults(String queryNum, String queryName,
		...
		if (model instanceof RetrievalModelRankedBoolean || model instanceof RetrievalModelBM25 || model instanceof RetrievalModelIndri ) {
			int count = 0;
			map = sort(treemap);
	 ...
	 * @throws IOException
	 */
	static String Unrankedprint(String queryNum, String queryName,
			QryResult result) throws IOException {
	...
	static String experimentPrint(String queryNum, String queryName,
		...
		List<Map.Entry<Integer, Double>> mappingList = new ArrayList<Map.Entry<Integer, Double>>(
				map.entrySet());
				...
				new Comparator<Map.Entry<Integer, Double>>() {
					public int compare(Map.Entry<Integer, Double> mapping1,
						...
						if ((int) (mapping2.getValue() - mapping1.getValue()) > 0)
							return 1;
						else if ((int) (mapping2.getValue() - mapping1
								.getValue()) < 0)
							return -1;
						else {
		 ...
		 * 
		 * 
		 * int count = 0; map = sort(treemap); for (String k : map.keySet()) {
		 * 
		 * s += queryNum + " " + "Q0 " + k + " " + (count + 1) + " " +
		 * treemap.get(k) + "00000000000 " + "fubar\n"; if (count == 99) {
		 * break; } count++; }
		 * 
		 * } else { int count = 0; for (String k : treemap.keySet()) {
		 * 
		 * s += queryNum + " " + "Q0 " + k + " " + (count + 1) + " " +
		 * map.get(k) + "00000000000 " + "fubar\n"; if (count == 99) { break; }
		 * count++; } }
